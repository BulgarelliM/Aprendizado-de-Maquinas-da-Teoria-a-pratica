...
class Resultado():
    ...
    def precisao(self) -> float:
        ...
        #dessa classe
        for classe in range(len(self.mat_confusao)): # Soma na coluna da classe predita o total de predições - soma tudo que foi predito como A (mesmo que na realidade seja C ou B)
    ...
    def revocacao(self) -> float:
        ...
        for classe in range(len(self.mat_confusao)): # soma linha. soma tudo que realmente é A, mesmo que tenha sifo predito como outra classe
            #por meio da matriz, obtem todos os elementos que são dessa classe
            num_classe = 0
            num_elementos_classe = 0
            for classe_prevista in range(len(self.mat_confusao)): # anda na linha
