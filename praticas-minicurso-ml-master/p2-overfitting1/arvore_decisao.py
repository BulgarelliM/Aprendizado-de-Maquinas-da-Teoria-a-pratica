...
def faz_classificacao(x_treino:pd.DataFrame, y_treino:pd.Series, x_teste:pd.DataFrame, y_teste:pd.Series, min_samples_split:float) -> Tuple[List[float],float]:
    ...
    if type(y_teste) is list:
        for i in y_teste:
            if (y_teste[j]) == y_predicted[j]:
                aux.append(1)
            else:
                aux.append(0)
            j=j+1
    else:    
        for i, item in y_teste.iteritems():
            while j< len(y_teste):
                if (y_teste.loc[ y_teste.index[j], i ]) == y_predicted[j]:
                    aux.append(1)
                else:
                    aux.append(0)
                j=j+1
 
    acuracia = np.sum(aux)/len(y_predicted)
